...
grep -Eq '\\[Safe Mode taken\\]|Taking Safe Mode session\\.\\.\\. Success!' tools/omega-router.sh || err 'RouterOS apply helper must accept verified Safe Mode confirmations'
...
grep -Fq 'send(proc, b"\\x04")' tools/routeros-safe-session.py || err 'Safe Mode driver must request Ctrl-D rollback on failure'
