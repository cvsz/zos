...
release-check: all
	...
	@test -f LICENSE || { echo "Refusing release: project-wide LICENSE is not declared." >&2; exit 2; }
	@test -z "$(git status --porcelain)" || { echo "Refusing release: working tree is dirty." >&2; exit 2; }
	@test "$(git branch --show-current)" = main || { echo "Refusing release: checkout must be main." >&2; exit 2; }
	@git fetch --quiet origin main
	@test "$(git rev-parse HEAD)" = "$(git rev-parse origin/main)" || { echo "Refusing release: local main is not origin/main." >&2; exit 2; }
