...
offline_contract_checks() {
  ...
  grep -Fqx 'ROUTER_LAN_CIDR=192.168.1.0/24' "$ROOT/config/topology.env.example" || disagree "topology.example LAN CIDR drift"
  grep -Fqx 'ROUTER_LAN_IP=192.168.1.1' "$ROOT/config/topology.env.example" || disagree "topology.example LAN gateway drift"
