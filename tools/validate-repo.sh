...
required=(
  ...
  tools/test-repo-security.sh tools/topology-reconcile.sh
...
grep -Fq 'OMEGA_PHASE_FILE:' tools/omega-router.sh || err 'RouterOS apply must mark each phase file in output'
grep -Fq 'transactional apply failed' tools/omega-router.sh || err 'RouterOS apply must have transactional error handler'
...
grep -Fq 'def enter_safe_mode(' tools/routeros-safe-session.py || err 'Safe Mode driver enter_safe_mode must be extracted for testability'
grep -Fq 'def release_safe_mode(' tools/routeros-safe-session.py || err 'Safe Mode driver release_safe_mode must be extracted for testability'
grep -Fq 'OMEGA_ALLOW_SAFE_MODE_UNROLL' tools/routeros-safe-session.py || err 'Safe Mode stale session must fail closed without OMEGA_ALLOW_SAFE_MODE_UNROLL'
grep -q 'test_enter_safe_mode_fail_closed_without_env_var' tools/test-routeros-safe-session.py || err 'Safe Mode regression must test fail-closed stale session without env var'
grep -q 'test_enter_safe_mode_unroll_with_permission' tools/test-routeros-safe-session.py || err 'Safe Mode regression must test unroll with OMEGA_ALLOW_SAFE_MODE_UNROLL'
grep -q 'test_release_safe_mode_normal' tools/test-routeros-safe-session.py || err 'Safe Mode regression must test release path'
grep -q 'test_release_safe_mode_sends_quit_after_release' tools/test-routeros-safe-session.py || err 'Safe Mode regression must test /quit after release'
grep -q 'test_rollback' tools/test-routeros-safe-session.py || err 'Safe Mode regression must test rollback'
grep -q 'test_never_quit_during_safe' tools/test-routeros-safe-session.py || err 'Safe Mode regression must verify /quit not sent during SAFE'
grep -Fq 'class SessionState' tools/routeros-safe-session.py || err 'Safe Mode driver must expose an explicit session state machine'
grep -Fq 'def generate_nonce(' tools/routeros-safe-session.py || err 'Safe Mode driver must generate a unique transaction nonce'
grep -Fq 'def build_framed_markers(' tools/routeros-safe-session.py || err 'Safe Mode driver must frame pass/fail markers with the nonce'
grep -Fq 'def classify_output(' tools/routeros-safe-session.py || err 'Safe Mode driver must classify output without trusting static PASS echo'
grep -Fq 'def next_action(' tools/routeros-safe-session.py || err 'Safe Mode driver must decide commit only on full success'
grep -Fq 'def sanitize_for_evidence(' tools/routeros-safe-session.py || err 'Safe Mode driver must redact secrets from evidence'
grep -q 'test_classify_ignores_static_pass_echo' tools/test-routeros-safe-session.py || err 'Safe Mode regression must prove static PASS echo is rejected'
grep -q 'test_next_action_commits_only_on_full_success' tools/test-routeros-safe-session.py || err 'Safe Mode regression must prove commit requires full success'
grep -q 'test_sanitize_redacts_secrets' tools/test-routeros-safe-session.py || err 'Safe Mode regression must prove evidence redaction'
...
grep -Fq ':do {' tools/omega-router.sh || err 'Safe Mode apply must wrap all phases in one RouterOS :do transaction'
...
grep -Fq 'OMEGA WG ROUTER PUBLIC KEY=' reinstall/OMEGA-RB4011-GOLDEN-REINSTALL.rsc || err 'golden reinstall must surface the regenerated router WireGuard public key'
grep -Fq '88:DC:96:55:58:E7 comment="RITRUECHAI-AP02"' reinstall/OMEGA-RB4011-GOLDEN-REINSTALL.rsc || err 'golden reinstall must include EnGenius .50-.58 inventory'
...
grep -Fq 'OMEGA GOLDEN REINSTALL VERIFY PASS' reinstall/OMEGA-RB4011-GOLDEN-REINSTALL.rsc || err 'golden reinstall must provide a fail-closed verification sentinel'
grep -Fq 'legacy 192.168.0.0/24 address remains' reinstall/OMEGA-RB4011-GOLDEN-REINSTALL.rsc || err 'golden reinstall verification must reject legacy LAN addressing'
grep -Fq 'LAN DHCP network contract failed' reinstall/OMEGA-RB4011-GOLDEN-REINSTALL.rsc || err 'golden reinstall must assert LAN DHCP gateway/DNS contract'
